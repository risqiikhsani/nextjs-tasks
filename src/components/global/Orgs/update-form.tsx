...
export default function UpdateForm({ data }: { data: OrganizationType }) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    console.log(`Updating Organization of id : ${data.id}`);
    try {
      console.log(values);
      const formData = new FormData()
      formData.append("name", values.name) 
      formData.append("description", values.description)
      await instance.put(`/api/orgs/${data.id}`, formData);
      RevalidatePath("/orgs");
      form.reset();
      setOpen(false);
